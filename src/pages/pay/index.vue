...
export default {
  ...
  methods: {
    async getPayInfo() {
      const result = await this.$api.trade.getOrderPayInfo(this.orderNo);
      if (result.code === 200) {
    ...
    // 立即支付
    payNow() {
      const codeUrl = this.payInfo.codeUrl;
      ...
      this.timer = setInterval(async () => {
        // 轮询订单支付状态接口
        const result = await this.$api.trade.getOrderPayStatus(this.orderNo);
        if (result.code === 200) {
          this.payState = result.code;
          clearInterval(this.timer);
          this.timer = null;
        }
      }, 2000);
