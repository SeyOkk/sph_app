<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        v-for="address in orderTrade.userAddressList"
        :key="address.id"
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="item in orderTrade.detailArrayList"
          :key="item.skuId"
        >
          <li>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b
            ><i>{{ orderTrade.totalNum }}</i
            >件商品，总商品金额</b
          >
          <span>¥{{ orderTrade.originalTotalAmount }}.00</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ orderTrade.totalAmount }}.00</span>
      </div>
...
export default {
  ...
  computed: {
    getDefaultAddress() {
      return (
        this.orderTrade.userAddressList.filter(
  ...
  methods: {
    async getOrderTrade() {
      const result = await this.$api.trade.getOrderTrade();
      if (result.code === 200) {
        this.orderTrade = result.data;
        // 设置默认的收货地址
    ...
    async submitOrder() {
      const data = {
        ...
        orderDetailList: this.orderTrade.detailArrayList,
      };
      const result = await this.$api.trade.submitOrder(
        this.orderTrade.tradeNo,
        data
      );
      if (result.code === 200) {
        // 路由跳转
